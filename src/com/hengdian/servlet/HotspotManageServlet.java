...
public class HotspotManageServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		System.out.println(spotName + "===" + longitudeStr + "===" + latitudeStr + "===" + excuteCodeStr);
		...
		if (longitudeStr.length() > 0 && latitudeStr.length() > 0) {
			...
			districtCode = JudgeDistrictsOfShenZhen.isInPolygon(
									new LoLatitude(latitude, longitude));
		}
		...
		if (0 != districtCode) {
			...
			case "add":
				sql = "select count(*) from hotspot_info where name=? or (longitude=? and latitude=?)";
				...
				try {
					while (rs.next()) {
						...
						if (rs.getInt(1) > 0) {
							...
							sql = "insert into hotspot_info values('"
								+spotName+"',"+longitude+","+latitude+","+districtCode+","
			...
			case "delete":
				...
				sql = "delete from hotspot_info where longitude=? and latitude=?";
				valArr = new Object[] { longitude, latitude };
				jdbcUtils.executeUpdateNotClose(sql, valArr);
				
				daoCode = 3;
				msg = "删除地点成功！经纬度：" + longitude + ","
						+ latitude;
				break;
			}
		} else {
		...
		if("delete".equals(excuteCodeStr)){
			String sql2 = "delete from key_point_area where lnglat=?";
			valArr = new Object[] { longitude + "-" + latitude };
			jdbcUtils.executeUpdateNotClose(sql2, valArr);	
		}
